...
async function main() {
  console.log(
    "🚀 Deploying Production ZK Language Model Verifiers to DevNet...",
  );
  ...
  for (const architecture of architectures) {
    try {
      ...
      const Verifier = await ethers.getContractFactory(
        `Production${architecture}Verifier`,
      );
      const Registry = await ethers.getContractFactory(
        `${architecture}InferenceRegistry`,
      );
      ...
      console.log(
        `   📊 Verifier Gas Estimate: ${verifierGasEstimate.toString()}`,
      );
      console.log(
        `   📊 Registry Gas Estimate: ${registryGasEstimate.toString()}`,
      );
      ...
      try {
        ...
        console.log(
          `   🔑 VK Alpha: [${vk.alpha[0].toString().slice(0, 10)}..., ${vk.alpha[1].toString().slice(0, 10)}...]`,
        );
        ...
        console.log(
          `   ✅ Test verification successful! Gas used: ${receipt.gasUsed}`,
        );
      } catch (testError) {
  ...
  if (Object.keys(deployedContracts).length > 0) {
    ...
    console.log(
      "🔗 Ready for testing and verification on the development network",
    );
  } else {
