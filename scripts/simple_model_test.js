...
async function main() {
  ...
  const deploymentData = JSON.parse(
    fs.readFileSync("fresh-devnet-deployment.json", "utf8"),
  );
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const proof = {
          ...
          t3: [
            `0x${"f".repeat(64)}`,
            `0x${(1234567890 + i).toString(16).padStart(64, "0")}`,
          ],
        ...
        try {
          ...
          console.log(
            `      ❌ Verification failed: ${error.message.split("\n")[0]}`,
          );
          archResults.push({
            sequence: i + 1,
            prompt: prefix,
      ...
      console.log(
        `   📊 ${architecture}: ${results[architecture].successCount}/3 successful`,
      );
    } catch (error) {
  ...
  console.log(
    `💰 Average Gas: ${totalVerifications > 0 ? (totalGas / BigInt(totalVerifications)).toString() : "N/A"}`,
  );
  ...
  const testData = {
    ...
    averageGas:
      totalVerifications > 0
        ? (totalGas / BigInt(totalVerifications)).toString()
        : "0",
    results: results,
  };

  fs.writeFileSync(
    "simple-model-test-results.json",
