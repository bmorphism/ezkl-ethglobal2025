...
async function main() {
  ...
  // Deploy Verifiers
  const rwkvVerifier = await hre.ethers.deployContract("ProductionRWKVVerifier");
  await rwkvVerifier.waitForDeployment();
  console.log(`  -> ProductionRWKVVerifier deployed to ${rwkvVerifier.target}`);

  const mambaVerifier = await hre.ethers.deployContract("ProductionMambaVerifier");
  await mambaVerifier.waitForDeployment();
  console.log(`  -> ProductionMambaVerifier deployed to ${mambaVerifier.target}`);

  const xlstmVerifier = await hre.ethers.deployContract("ProductionxLSTMVerifier");
  await xlstmVerifier.waitForDeployment();
  console.log(`  -> ProductionxLSTMVerifier deployed to ${xlstmVerifier.target}`);
  ...
  const proofChainOrchestrator = await hre.ethers.deployContract("ProofChainOrchestrator");
  await proofChainOrchestrator.waitForDeployment();
  console.log(`  -> ProofChainOrchestrator deployed to ${proofChainOrchestrator.target}`);
