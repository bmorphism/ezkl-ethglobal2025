...
async function main() {
  ...
  const deployments = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "../config/devnet-deployment-addresses.json"),
    ),
  );
  ...
  const proof = fs.readFileSync(
    path.join(__dirname, "../ezkl_workspace/rwkv_simple/proof.json"),
    "utf8",
  );
  ...
  const inputData = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "../ezkl_workspace/rwkv_simple/input.json"),
      "utf8",
    ),
  );
